...
if sudo nft list tables | grep -q "fail2ban"; then
    ...
    sudo nft list table inet fail2ban-filter > /tmp/nftables-backup-$(date +%s).nft 2>/dev/null || true
...
# Export tabuľky
sudo nft list table inet fail2ban-filter > /tmp/fail2ban-filter.nft
