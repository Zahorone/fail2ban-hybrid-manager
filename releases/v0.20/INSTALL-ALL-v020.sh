...
╔════════════════════════════════════════════════════════════════╗
║                                                                ║
║      Fail2Ban + nftables Complete Setup v0.20                 ║
...
if nft list table inet fail2ban-filter &>/dev/null 2>&1; then
    ...
    CURRENT_SETS_V4=$(nft list table inet fail2ban-filter 2>/dev/null | grep "^\s*set f2b-" | grep -v '\-v6' | wc -l || echo 0)
    CURRENT_SETS_V6=$(nft list table inet fail2ban-filter 2>/dev/null | grep "^\s*set f2b-.*-v6" | wc -l || echo 0)
...
case $INSTALL_TYPE in
    fresh)
        ...
        echo "  • F2B wrapper v0.20 (43 functions)"
    ...
    reinstall)
        echo "REINSTALL - v0.20 already present"
        echo ""
...
if [ -f "$SCRIPT_DIR/scripts/00-pre-cleanup-v020.sh" ]; then
    bash "$SCRIPT_DIR/scripts/00-pre-cleanup-v020.sh" || warning "Pre-cleanup had warnings (continuing)"
else
...
if [ -f "$SCRIPT_DIR/scripts/01-install-nftables-v020.sh" ]; then
    bash "$SCRIPT_DIR/scripts/01-install-nftables-v020.sh" || error "nftables installation failed"
else
...
if [ -f "$SCRIPT_DIR/scripts/02-install-jails-v020.sh" ]; then
    bash "$SCRIPT_DIR/scripts/02-install-jails-v020.sh" || error "Jails installation failed"
else
...
step 5 $TOTAL_STEPS "Installing F2B wrapper v0.20 (43 functions)"
echo ""

# Try direct wrapper installation first
if [ -f "$SCRIPT_DIR/scripts/f2b-wrapper-v020.sh" ]; then
    sudo cp "$SCRIPT_DIR/scripts/f2b-wrapper-v020.sh" /usr/local/bin/f2b
    sudo chmod +x /usr/local/bin/f2b
    log "F2B wrapper installed at /usr/local/bin/f2b"
elif [ -f "$SCRIPT_DIR/scripts/04-install-wrapper-v020.sh" ]; then
    bash "$SCRIPT_DIR/scripts/04-install-wrapper-v020.sh" || error "Wrapper installation failed"
else
...
# Verify nftables
SETS_V4=$(nft list table inet fail2ban-filter 2>/dev/null | grep "^\s*set f2b-" | grep -v '\-v6' | wc -l || echo 0)
SETS_V6=$(nft list table inet fail2ban-filter 2>/dev/null | grep "^\s*set f2b-.*-v6" | wc -l || echo 0)
...
if [ $ERRORS -eq 0 ]; then
    echo "Your system is now protected with v0.20:"
