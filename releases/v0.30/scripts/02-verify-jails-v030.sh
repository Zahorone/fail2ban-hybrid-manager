...
if sudo nft list table inet fail2ban-filter &>/dev/null; then
    ...
    echo "  Sets found: $SETS_COUNT / 22 expected (11 IPv4 + 11 IPv6)"
    
    # Check INPUT chain
    INPUT_RULES=$(sudo nft list chain inet fail2ban-filter f2b-input 2>/dev/null | grep -c "drop" || echo 0)
    echo "  INPUT rules: $INPUT_RULES / 22 expected"
...
if [ "$TOTAL_JAILS" -lt 10 ]; then
    log_warn "Only $TOTAL_JAILS jails active (expected: 10+)"
    ((ISSUES++))
else
...
if [ "$SETS_COUNT" -eq 22 ]; then
    log_success "All nftables sets present (22/22)"
else
    log_warn "nftables sets incomplete ($SETS_COUNT/22)"
    ((ISSUES++))
fi
