...
################################################################################
# F2B Wrapper v0.33 Installation Script
# Installs the unified F2B management wrapper
#
# v0.33 KEY CHANGES (2026-01-01):
#   - Updated compatibility for nftables v0.33 (12 jailov, 24 setov)
#   - Improved version/release matching against f2b-wrapper-v033.sh
...
WRAPPER_SOURCE=""
if   [ -f "$SCRIPT_DIR/f2b-wrapper-v033.sh" ]; then
  WRAPPER_SOURCE="$SCRIPT_DIR/f2b-wrapper-v033.sh"
elif [ -f "$SCRIPT_DIR/f2b-wrapper.sh" ]; then
  WRAPPER_SOURCE="$SCRIPT_DIR/f2b-wrapper.sh"
elif [ -f "$HOME/f2b-wrapper-v033.sh" ]; then
  WRAPPER_SOURCE="$HOME/f2b-wrapper-v033.sh"
else
  error "F2B wrapper script not found. Please ensure f2b-wrapper-v033.sh is in $SCRIPT_DIR"
fi
...
echo "═══════════════════════════════════════════════════════"
echo ""
log "Wrapper compatible with v0.33 (12 jails, 24 nftables setov)"
