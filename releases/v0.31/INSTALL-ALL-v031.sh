...
################################################################################
# Fail2Ban + nftables v0.31 - Universal Installer
...
# - Reinstall v0.23 (rebuild components)
################################################################################
...
echo ""
if [ -f "$SCRIPTDIR/scripts/00-pre-cleanup-v031.sh" ]; then
  if [ "$FORCE_CLEANUP" = "yes" ]; then
    info "Pre-cleanup: FORCE mode enabled (--clean-install/--force-cleanup)"
    F2B_FORCE_CLEANUP=yes bash "$SCRIPTDIR/scripts/00-pre-cleanup-v031.sh" \
      || warning "Pre-cleanup had warnings (continuing)"
  else
    bash "$SCRIPTDIR/scripts/00-pre-cleanup-v031.sh" \
      || warning "Pre-cleanup had warnings (continuing)"
  fi
else
...
echo ""
if [ -f "$SCRIPTDIR/scripts/01-install-nftables-v031.sh" ]; then
    bash "$SCRIPTDIR/scripts/01-install-nftables-v031.sh" || error "nftables installation failed"
else
...
echo ""
if [ -f "$SCRIPTDIR/scripts/02-install-jails-v031.sh" ]; then
    bash "$SCRIPTDIR/scripts/02-install-jails-v031.sh" || error "Jails installation failed"
else
...
echo ""
if [ -f "$SCRIPTDIR/scripts/05-install-auto-sync-v031.sh" ]; then
    bash "$SCRIPTDIR/scripts/05-install-auto-sync-v031.sh" || warning "Auto-sync installation had warnings"
else
...
echo ""
if [ -f "$SCRIPTDIR/scripts/06-install-aliases-v031.sh" ]; then
    bash "$SCRIPTDIR/scripts/06-install-aliases-v031.sh" || warning "Aliases installation had warnings"
else
...
echo ""
if [ -f "$SCRIPTDIR/scripts/07-setup-docker-sync-cron-v031.sh" ]; then
    bash "$SCRIPTDIR/scripts/07-setup-docker-sync-cron-v031.sh" || warning "Docker-sync cron setup had warnings"
else
