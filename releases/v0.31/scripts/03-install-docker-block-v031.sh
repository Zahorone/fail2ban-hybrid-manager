...
echo ""
echo "══════════════════════════════════════════════════════════"
echo " Docker Port + IP Blocking Setup v0.4 (v0.31)"
echo "══════════════════════════════════════════════════════════"
echo ""
...
info "Step 1: Creating /etc/nftables.d/docker-block.nft..."

# CRITICAL: mkdir MUST come FIRST
mkdir -p /etc/nftables.d

cat > /etc/nftables.d/docker-block.nft << 'EOF'
#!/usr/sbin/nft -f
...
chown root:root /etc/nftables.d/docker-block.nft
chmod 644 /etc/nftables.d/docker-block.nft

log "/etc/nftables.d/docker-block.nft created (with IP blocking)"
...
if [ ! -f /etc/nftables.conf ]; then
  ...
  if ! grep -q "/etc/nftables.d/docker-block.nft" /etc/nftables.conf; then
...
# Fail2Ban nftables (v0.30 - IPv4/IPv6, recidive 30d + FORWARD)
...
EOF
  ...
  info "Add: include \"/etc/nftables.d/docker-block.nft\""
...
if nft -f /etc/nftables.d/docker-block.nft 2>&1; then
...
echo " • /etc/nftables.d/docker-block.nft (v0.4 - with IP blocking)"
