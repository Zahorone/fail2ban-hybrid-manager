# Fail2Ban + nftables v0.31 - Production Bundle

Complete Fail2Ban + nftables integration with one-click installer, full IPv4 + IPv6 dual-stack support, 11 advanced detection filters, Docker port and IP blocking with immediate bans, and a management wrapper v0.32 with extended reporting and docker-block integration.

## Features

### Core Infrastructure

- **Full IPv4 + IPv6 Support** - 22 nftables sets (11 IPv4 + 11 IPv6) in table inet fail2ban-filter
- **11 Fail2Ban Jails** - Multi-layered security (sshd, exploit, DoS, web, recon, fuzzing, botnet, anomaly, recidive, manualblock)
- **11 Detection Filters** - Optimized filters for each jail
- **30d Recidive Jail** - Long-term repeat offender blocking
- **Safe Pre-Cleanup** - Backup and legacy cleanup before install

### Advanced Tools

- **One-Click Installer** - INSTALL-ALL-v031.sh orchestrates full install and upgrades from v0.19–v0.30
...
- **F2B Wrapper v0.32** - 50+ management functions with attack analysis, reports, and improved locking
...
10. **manualblock** - Manual IP banning jail
11. **recidive** - Repeat offenders with 30-day ban
...
- f2b-anomaly-detection.conf, manualblock.conf, recidive.conf

Two local filter tuning files extend behaviour via ignoreregex and custom patterns: nginx-recon-optimized.local and f2b-anomaly-detection.local.
...
- **Validate cron path:**
  - 07-setup-docker-sync-cron-v031.sh installs root cron to run f2b docker sync validate every minute
...
03-install-docker-block-v031.sh creates table inet docker-block with:
...
### One-Command Installation (v0.31)

```bash
tar -xzf f2b-hybrid-nftables-v031.tar.gz
cd v031
...
- Upgrade from v0.19–v0.30 (preserves bans and structure)
- Reinstall v0.31 (rebuild with ban preservation)
...
This runs 00-pre-cleanup-v031.sh with backup and legacy cleanup, then exits before modifying nftables or jails.
...
Expected: 22 drop rules (11 IPv4 + 11 IPv6) and all 11 jails active.

## Repository Structure (v0.31)

```
v031/
├── INSTALL-ALL-v031.sh
├── 00-pre-cleanup-v031.sh
├── 01-install-nftables-v031.sh
├── 02-install-jails-v031.sh
├── 02-verify-jails-v031.sh
├── 03-install-docker-block-v031.sh
├── 04-install-wrapper-v031.sh
├── 05-install-auto-sync-v031.sh
├── 06-install-aliases-v031.sh
├── 07-setup-docker-sync-cron-v031.sh
├── f2b-wrapper-v031.sh
├── f2b-docker-hook.sh
...
## What Is New in v0.31

- One-click installer updated: INSTALL-ALL-v031.sh for v0.31 layout and docker-hook integration
- Wrapper upgraded from v0.30 to v0.32 with better locking, IPv6 validation, jq helpers and extended reports
- Immediate docker-block ban path via f2b-docker-hook.sh and docker-sync-hook.conf
- Docker-block cron switched to validation mode using f2b docker sync validate
- Infrastructure counts unchanged (11 jails, 11 filters, 11+11 sets, 22 INPUT, 6 FORWARD) for safe upgrade from v0.30
...
- Verification tool: sudo bash 02-verify-jails-v031.sh
